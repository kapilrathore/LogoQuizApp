...
protocol LogoProviderType {
    func fetchLogos() -> [Logo]
}

class LogoProvider: LogoProviderType {
    func fetchLogos() -> [Logo] {
        let logoJsonString = """
            [
                {
        ...
        if let jsonData = logoJsonString.data(using: .utf8),
           let logos = try? JSONDecoder().decode([Logo].self, from: jsonData) {
            return logos
        }
